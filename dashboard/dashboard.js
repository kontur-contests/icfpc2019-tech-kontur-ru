...
Promise.all([algsRequest, inProgressRequest, blockchainRequest]).then(([data, inProg, blockchain]) => {
    ...
    const tableHtml = `<head>${styles}${metaGeneral}${meta}</head><body>${dataForScript}${progressDataForScript}${blockchainDataForScript}${scripts}</body>`;
