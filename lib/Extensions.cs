...
namespace lib
{
    public static class Extensions
    {
        ...
        public static bool Inside<T>(this V v, Map<T> map)
        {
            return 0 <= v.X && v.X < map.SizeX && 0 <= v.Y && v.Y < map.SizeY;
        }
        ...
        public static V RotateAroundZero(this V p, int rotationIndex)
        {
            ...
            switch (rotationIndex)
            {
                case 0:
                    return p;
                case 1:
                    return new V(p.Y, -p.X);
                case 2:
                    return new V(-p.X, -p.Y);
                case 3:
                    return new V(-p.Y, p.X);
                default:
                    throw new Exception();
        ...
        public static bool IsReachable(this Map map, V src, V dest)
        {
            ...
            if (Math.Abs(dy) == Math.Abs(dx))
            {
                ...
                {
                    if (map[new V(x, y)] == CellState.Obstacle)
                        return false;
                }
            }
            ...
            {
                if (dy < 0)
                    return IsReachable(map, dest, src);
                var nx = 2 * dy * x0 + dx + dy;
                for (int y = y0 + 1; y < y1; y++)
                {
                    var x = nx / (2 * dy);
                    if (nx % (2 * dy) != 0 && map[new V(x, y)] == CellState.Obstacle)
                        return false;
                    nx += 2 * dx;
                    x = nx / (2 * dy);
                    if (nx % (2 * dy) != 0 && map[new V(x, y)] == CellState.Obstacle)
            ...
            else
            {
                ...
                {
                    if (ny % (2 * dx) != 0 && map[new V(x, ny / (2 * dx))] == CellState.Obstacle)
                        return false;
                    ny += 2 * dy;
                    if (ny % (2 * dx) != 0 && map[new V(x, ny / (2 * dx))] == CellState.Obstacle)
                        return false;
